...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fetch initial apparatus
    function fetchApparatus(page) {
        fetch(`/apparatus?page=${page}`)
            .then(response => response.json())
            .then(data => {
                // // Clear previous apparatus
                // apparatusContainer.innerHTML = '';
                displayApparatus(data.apparatus);
    ...
    // Display apparatus on the page
    function displayApparatus(apparatus) {
        apparatusContainer.innerHTML = ''; // Clear previous apparatus

        apparatus.forEach(apparatus => {
            const apparatusElement = document.createElement('li');
            apparatusElement.classList.add('product-item', 'col-md-4', 'col-sm-4', 'col-xs-6', 'col-lg-4');
            apparatusElement.innerHTML = `
            <div class="contain-product layout-default">
                <div class="product-thumnail">
                    <a href="#" class="link-to-product">
                        <img src="${apparatus.image_url}" alt="${apparatus.name}">
                    </a>    
                </div>
                <div class="info">
                <b class="categories">${apparatus.category}</b>
                <h4 class="product-title"><a href="#" class="pr-name">${apparatus.name}</a></h4>
                <div class="price">
                <ins><span class="price-amount"><span class="currencySymbol">KES</span>${apparatus.price}</span></ins>
                </div>
                <div class="slide-down-box">
                    <p class="message">${apparatus.category}</p>
                    <div class="buttons">
            ...
            apparatusContainer.appendChild(apparatusElement);
        });
    }
...
function getImagePath(imageUrl) {
    ...
    // return imageUrl.replace('/images/apparatus', '/apparatus');
}
...
async function loadGridApparatus(page = 1) {
    try {
        const response = await fetch(`/apparatus?page=${page}&limit=12`);
        const data = await response.json();
        ...
        // Create row wrapper for every 3 apparatus
        let currentRow;
        data.apparatus.forEach((apparatus, index) => {
            // Create new row for every 3rd apparatus
            if (index % 3 === 0) {
            ...
            const apparatusHTML = `
                <li class="product-item col-lg-4 col-md-4 col-sm-4 col-xs-6">
                    <div class="contain-product layout-default">
                        <div class="product-thumb">
                            <a href="#" class="link-to-product">
                                <img src="${getImagePath(apparatus.image_url)}" 
                                     alt="${apparatus.name}" 
                                     width="270" 
                                     height="270" 
                                     class="product-thumnail">
                        ...
                        <div class="info">
                            ...
                            <div class="shipping-info">
                                <p class="shipping-day">${apparatus.publishers || ''}</p>
                            </div>
                            <div class="slide-down-box">
                                <div class="buttons">
            ...
            `;
            currentRow.insertAdjacentHTML('beforeend', apparatusHTML);
        });
        ...
        console.error('Error loading grid apparatus:', error);
    }
}
